...
function showFormSuccess(form) {
  const container = form.closest(
    ".hero-quick-card, .deck-form-card"
  );

  const overlay = container?.querySelector(
    "[data-success-overlay]"
  );
...
function clearInvalidState(control) {
  ...
  control
    .closest("fieldset")
    ?.classList.remove("field-invalid-group");
}
  ...
  for (const control of controls) {
    ...
    if (control.type === "radio") {
      ...
      if (isRequired && !hasSelection) {
        group[0]
          ?.closest("fieldset")
          ?.classList.add("field-invalid-group");
...
function getCheckedValues(form, fieldName) {
  return Array.from(
    form.querySelectorAll(
      `input[type="checkbox"][name="${CSS.escape(
        fieldName
      )}"]:checked`
    )
  ).map((input) =>
...
function scoreLead(details) {
  ...
  if (
    details.projectTiming === "As soon as practical" ||
    details.projectTiming === "This summer" ||
    details.projectTiming === "1-3 months"
  ) {
    score += 2;
    signals.push("Near-term project");
  } else if (details.projectTiming) {
    score += 1;
    signals.push("Longer-term project");
  }

  if (
    details.budgetRange === "$10,000-$20,000" ||
    details.budgetRange === "$20,000-$35,000" ||
    details.budgetRange === "$35,000+"
  ) {
    score += 3;
    signals.push("Higher stated budget");
  } else if (
    details.budgetRange === "$5,000-$10,000"
  ) {
    score += 2;
    signals.push("Mid-range stated budget");
  } else if (details.budgetRange) {
    score += 1;
    signals.push("Budget needs review");
  }

  if (
    details.currentSetup ===
      "No deck / grass outside" ||
    details.currentSetup ===
      "Existing deck to remove"
  ) {
    score += 1;
...
function initializeMultiStepForm(form) {
  ...
  const steps = Array.from(
    form.querySelectorAll(".estimate-step")
  );
  ...
  const progressTitle = form.querySelector(
    "[data-progress-title]"
  );
  ...
  const clearFilesButton = form.querySelector(
    "[data-clear-files]"
  );
  ...
  function validateStep(step) {
    ...
    const invalidControl =
      findInvalidControl(step);

    if (!invalidControl) {
      return true;
    }

    invalidControl.classList.add(
      "field-invalid"
    );
  ...
  form.addEventListener(
    ...
    async (event) => {
      ...
      if (
        invalidControl instanceof HTMLElement
      ) {
        const invalidStep =
          invalidControl.closest(
            ".estimate-step"
          );

        const invalidStepIndex =
          steps.indexOf(invalidStep);
      ...
      const details = {
        ...
        materialPreference: getRadioValue(
          formData,
          "materialPreference"
        ),

        deckSize: String(
          formData.get("deckSize") || ""
        ).trim(),

        desiredFeatures: getCheckedValues(
          form,
          "desiredFeatures"
        ),

        projectTiming: String(
          formData.get("projectTiming") ||
            ""
        ).trim(),

        budgetRange: getRadioValue(
          formData,
          "budgetRange"
        ),
        ...
        contactPreference: getRadioValue(
          formData,
          "contactPreference"
        ),
      ...
      const description = [
        ...
        `Desired features: ${
          details.desiredFeatures.join(", ") ||
          "None selected"
        }`,

        `Ideal timing: ${details.projectTiming}`,

        `Approximate project budget: ${details.budgetRange}`,

        `Preferred contact method: ${details.contactPreference}`,

        `Confirmed new deck construction: ${
          details.newDeckConfirmation
            ? "Yes"
            : "No"
        }`,
      ...
      try {
        ...
        pushTrackingEvent(
          "deck_lead_submit",
          {
            ...
            homeowner_status:
              details.homeownerStatus,

            project_zip:
              details.zipCode,

            material_preference:
              details.materialPreference,

            deck_size:
              details.deckSize,

            project_timing:
              details.projectTiming,
...
function initializeServiceCarousel() {
  ...
  controls.innerHTML = `
    <button
      class="services-carousel-arrow services-carousel-prev"
      type="button"
      aria-label="Previous deck service"
    >‹</button>

    <div
      class="services-carousel-dots"
      aria-label="Deck service navigation"
    ></div>

    <button
      class="services-carousel-arrow services-carousel-next"
      type="button"
      aria-label="Next deck service"
    >›</button>
  `;
...
function replaceVisibleLinkText(
  link,
  label
) {
  if (!link) {
